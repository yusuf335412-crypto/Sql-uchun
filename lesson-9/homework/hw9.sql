...
--q1
--Using Products, Suppliers table List all
--combinations of product names and supplier names.

select * from Suppliers
select * from Products

select productname, suppliername from Suppliers p cross join Products s 

--q2
--Using Departments, Employees table Get all 
--combinations of departments and employees.

select * from Departments
select* from Employees
select  name, DepartmentName from Departments d cross join Employees e

--q3
--Using Products, Suppliers table List only the combinations
--where the supplier actually supplies the product.
...
select firstname+lastname as full_name, orderid from 
...
select name,coursename from Courses c cross join Students s
 ...
 --Using Departments, Employees table
 --List
 --employees whose DepartmentID matches the department.

 select* from Departments
 select* from Employees

 select * from Departments d join Employees e on d.DepartmentID=e.DepartmentID

 --q8
 --Using Students, Enrollments table List student names and their enrolled course IDs.
select* from Students
select * from Enrollments
select name, enrollmentid from Students s join Enrollments e on s.StudentID=e.StudentID
...
select* from Orders
select * from Products
Select productname, orderid,Price from Orders o join Products p on o.ProductID=p.ProductID where Price>100

--Medium (10 puzzles)
--q11
--Using Employees, Departments table List employee names and 
--department names where department IDs are not equal. 
--It means: Show all mismatched employee-department combinations.

select * from Employees
select* from Departments

select DepartmentName,name from Departments d right join Employees e on d.DepartmentID=e.DepartmentID
select DepartmentName,name from Departments d left join Employees e on d.DepartmentID=e.DepartmentID


--q12
--Using Orders, Products table Show orders where ordered
...
--q13
--Using Customers, Sales table List customer names and 
--product IDs where sale amount is 500 or more.

Select* from Customers
select * from Sales

select* from Customers c join sales s on c.CustomerID=s.CustomerID where SaleAmount>500

--q14
--Using Courses, Enrollments, Students table List student names and
...
--Q15
--Using Products, Suppliers table List product and supplier 
--names where supplier name contains “Tech”.
SELECT* FROM Products
SELECT*FROM Suppliers
...
--Q16
--Using Orders, Payments table Show orders where payment 
...
--Q17
--Using Employees and Departments tables, get the Department 
...
--Q18
--Using Products, Categories table Show products where 
--category is either 'Electronics' or 'Furniture'.
...
--q20
--Using Orders, Customers table List orders made 
--by customers from 'Germany' and order total > 100.
select* from Orders
select* from Customers
select orderid, country, totalamount from Orders o join Customers c
on o.CustomerID=c.CustomerID where Country='germany' and TotalAmount>100

--q21
--Using Employees table List all pairs of employees from different departments.
select * from Employees
select * from Employees e1 join Employees e2  on 
e1.DepartmentID<>e2.DepartmentID where e1.DepartmentID<e2.DepartmentID


--q22
--Using Payments, Orders, Products table List payment details 
--where the paid amount 
--is not equal to (Quantity × Product Price).
select * from Payments
select* from Orders
select* from Products
select Quantity*Price, Amount from Payments p join Orders o on p.OrderID=o.Orderid 
join Products pr on pr.ProductID=o.ProductID where Amount<>Quantity*Price
...
select* from Students s left join Enrollments e on s.StudentID=e.StudentID
 left join Courses c on  e.courseid=c.CourseID where  EnrollmentID is null

 --q24
--Using Employees table List employees
--who are managers of someone, but their salary is less than or
--equal to the person they manage.
select * from Employees
select * from Employees emp left join 
Employees man on emp.managerid=man.EmployeeID 
where man.Salary<emp.Salary  or man.Salary=emp.Salary 

--q25
--Using Orders, Payments, Customers table List customers
--who have made an order, 
--but no payment has been recorded for it.
select * from Orders
select * from Payments
select * from Customers
Select firstname+lastname, Amount from Customers c left join Orders o on c.CustomerID=o.customerid 
left join Payments p on p.OrderID=o.orderid where Amount is null
