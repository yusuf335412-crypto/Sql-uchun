...
go
use hw13
create table Employees_full (EMPLOYEE_ID int, FIRST_NAME varchar(50), LAST_NAME varchar(50),EMAIL varchar(255), PHONE_NUMBER varchar(50),HIRE_DATE date,JOB_ID varchar(50),SALARY float,COMMISSION_PCT float, MANAGER_ID int, DEPARTMENT_ID int)

insert into Employees_full values
...
INSERT INTO Employee_title(EmployeeID, ManagerID, JobTitle) VALUES
...
--q1
/*You need to write a query that outputs "100-Steven King", 
meaning emp_id + first_name + last_name in that format using employees table*/



select concat(employee_id,  '-',first_name, ' ', last_name) from Employees_full

--q2
--Update the portion of the phone_number in the employees table, 
--within the phone number the substring '124' will be replaced by '999'
select * from Employees_full

select *, REPLACE(phone_number,'124', '999') [replaced phone numbers] from Employees_full

--q3
--Display the first name and the length of the first name for all employees 
...
select * from Employees_full
select first_name, len(first_name) from Employees_full 
where FIRST_NAME like'a%' or FIRST_NAME like 'j%' or FIRST_NAME like 'm%'

--q4
--Write an SQL query to find the total salary for each manager ID.(Employees table)

select MANAGER_ID, sum(salary) from Employees_full group by MANAGER_ID

--q5
--Write a query to retrieve the year and the highest value from the columns Max1, Max2,
...
SELECT *,
       CASE WHEN Id = 0 THEN 1 ELSE 0 END AS IsZero
FROM SingleOrder
ORDER BY IsZero, Id asc;
...
--If the first column is null, move to the next,
--and so on. If all columns are null, return null.(person)
...
len(fullname) -charindex(' ', fullname,CHARINDEX(' ', fullname)+1)) from  Students
...
--q4
--Find all employees whose names (concatenated first and last) contain
--the letter "a" at least 3 times.
select * from Employees
SELECT 
    CONCAT(first_name, ' ', last_name) AS full_name
FROM Employees
WHERE 
    (dataLENGTH(CONCAT(first_name, last_name)) 
     - dataLENGTH(REPLACE(CONCAT(first_name, last_name), 'a', ''))) >= 3;
	   
--q5

--The total number of employees in each department and the
--percentage of those employees who have been with the company 
...
--q1
--Write an SQL query that replaces each row with the sum of its value 
--and the previous rows' value. (Students table)

select * from Students

select Studentid,
        grade, sum(grade) over (order by studentid)
...
select  * from Student a join student b on a.birthday=b.Birthday where a.StudentName<>b.StudentName
...
select 
   case 
   when PlayerA < PlayerB then PlayerA else playerb end  player1,
   case 
   when playerA<playerB then playerB else playerA end  player2,
   sum(score)  total
   ...
   group by 
   case 
   when PlayerA < PlayerB then PlayerA else playerb end,  
   case 
   when playerA<playerB then playerB else playerA end  
...
from (
 select 
	   ...
	   CASE
	   WHEN
	      PlayerA<PlayerB then Playerb else playera end player2,
		  score
		  from PlayerScores) p 
