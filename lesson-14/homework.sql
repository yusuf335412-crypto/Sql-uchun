...
--Easy Tasks
--q1
--Write a SQL query to split the Name column by a comma into two separate columns: 
...
SELECT 
    s.id,
    v.value AS SplitPart
FROM Splitter s
CROSS APPLY STRING_SPLIT(s.Vals, '.') AS v;
...
select id, count(splitpart) as dots from
(select id, s.value as splitpart from testDots t cross apply string_split(t.vals,'.') as s ) x 

group by id

having count(splitpart)>2
...
SELECT SUM(UZUNLIK) FROM 

(SELECT texts,LEN(TEXTS)-LEN(REPLACE(texts,' ','')) AS UZUNLIK FROM CountSpaces) AS U
...
select * from Employee e1 join Employee e2 on e1.ManagerId=e2.Id where e1.Salary>e2.Salary

--Q7
--Find the employees who have been with the company for more than 10 years, 
...
SELECT EMPLOYEE_ID,FIRST_NAME,LAST_NAME,HIRE_DATE FROM EmployeeS WHERE  DATEDIFF(YEAR,HIRE_DATE,GETDATE()) BETWEEN 10 AND 15
...
SELECT * from weather w1 join weather w2 on DATEDIFF(day,w1.RecordDate,w2.RecordDate)=1
...
FROM Activity ) a where row_num=1 
...
from fruits f cross apply string_split(f.fruit_list,',') list ) dr where rn=3


--q4
--Write an SQL query to determine the Employment Stage for each employee based on their HIRE_DATE. 
...
select *,
   ...
   when datediff(year,hire_date, getdate())>=5 and datediff(year,hire_date, getdate())<10  then 'mid-level'
	...
	when datediff(year,hire_date, getdate())>=20 then 'veteran' else null
   end yillar
  from Employees

--q5
--Write a SQL query to extract the integer value that
--appears at the start of the string in a column named Vals.(GetIntegers)

select* from GetIntegers


select * from GetIntegers where VALS like '[0-9]%'
...
select id,
 ...
 --q2
 --Write a SQL query to create a table where each character from 
 --the string will be converted into a row.(sdgfhsdgfhs@121313131)
...
select player_id, device_id, event_date, rn
from

(select player_id, device_id, event_date,

    ROW_NUMBER() over (partition by device_id order by event_date asc) as rn from Activity)  vj
	where rn=1
