...
--Task: Show all orders placed after 2022 along with 
--the names of the customers who placed them.
--Tables Used: Orders, Customers

Select * from Orders
select * from Customers

select FIRSTNAME+LastName, OrderID, OrderDate from Orders o join Customers c on o.CustomerID=c.CustomerID where YEAR ( OrderDate) >2022 
...
SELECT * FROM Employees E JOIN Departments D ON E.DepartmentID=D.DepartmentID and DepartmentName='sales' or DepartmentName='marketing'
...
select * FROM Departments
SELECT * from Employees
select departmentname, max(salary) as maxsalary from Departments d join Employees e on d.DepartmentID=e.DepartmentID 
...
--and Country='USA'  AND  YEAR(OrderDate)=2023
...
select firstname, sum (quantity) as total from Orders o join Customers c 
on o.CustomerID=c.CustomerID group by FirstName


--q6
--Return: ProductName, SupplierName
--Task: Display the names of products that are supplied 
...
select * FROM Products P JOIN Suppliers S ON P.SupplierID=S.SupplierID AND SupplierName='GADGET SUPPLIES' 
OR SupplierName='CLOTHING MART'
select * FROM Products P JOIN Suppliers S ON P.SupplierID=S.SupplierID WHERE SupplierName='GADGET SUPPLIES' 
OR SupplierName='CLOTHING MART'
--q7
--Return: CustomerName, MostRecentOrderDate
--Task: For each customer, show their most recent order. 
--Include customers who haven't placed any orders.
Tables Used: Customers, Orders
select * from Orders
select* from Customers

select c.customerid, max(orderdate) most_recent from Customers c left join Orders o on c.CustomerID=o.CustomerID group by c.CustomerID
 
-- q8
-- Return: CustomerName, OrderTotal
--Task: Show the customers who have placed 
--an order where the total amount is greater than 500.
--Tables Used: Orders, Customers

select* FROM Orders
SELECT * FROM Customers

SELECT firstname as CustomerName, totalamount as OrderTotal from Customers c join Orders o  on c.CustomerID=o.CustomerID where TotalAmount>500


-- q9
--Return: ProductName, SaleDate, SaleAmount
--Task: List product sales where the sale was made in 2022
--or the sale amount exceeded 400.
--Tables Used: Products, Sales
select * from Products
select * from Sales

select ProductName, SaleDate, SaleAmount from Products p join Sales s on p.ProductID=s.ProductID 
where Year(SaleDate)=2022 and SaleAmount>400

--q10
--Return: ProductName, TotalSalesAmount
--Task: Display each product along with 
--the total amount it has been sold for.
--Tables Used: Sales, Products
select * from Sales
select * from Products
select ProductName, sum(saleamount) from Products p join Sales s on p.ProductID=s.ProductID
group by ProductName

--q11
--Return: EmployeeName, DepartmentName, Salary
--Task: Show the employees who work in the HR department 
--and earn a salary greater than 60000.
--Tables Used: Employees, Departments

select * from Employees
select * from Departments
select name as EmployeeName, DepartmentName, Salary from Employees e join Departments d on e.DepartmentID=d.DepartmentID
...
select* from Products
select* from Sales
select ProductName, SaleDate, StockQuantity from Products p join Sales s on p.ProductID=s.ProductID where YEAR(saledate)=2023 and StockQuantity>100
...
select Name, DepartmentName, HireDate from Employees e join Departments d on e.DepartmentID=d.DepartmentID 
...
select FirstName, OrderID, Address, OrderDate from Customers c join Orders o on c.CustomerID=o.CustomerID 
where Country='usa' and Address like '[0-9][0-9][0-9][0-9]%'

--q15
--Return: ProductName, Category, SaleAmount
--Task: Display product sales for items in the Electronics category 
--or where the sale amount exceeded 350.
--Tables Used: Products, Sales

select* from Products
select* from Sales

select ProductName,Category, SaleAmount from products p join sales s on p.productid=s.productid 
where Category='elelctronics' or SaleAmount>350
...
select  category, count(category) from Products p join Categories c on p.Category=c.CategoryID
group by Category
...
select* from Customers
select * from Orders
select FirstName+LastName as full_NAME, City,OrderID,TotalAmount AS TOTAL from Customers c join Orders o on c.CustomerID=o.CustomerID
where City='los angeles' or TotalAmount>300

--Q18
--Return: EmployeeName, DepartmentName
--Task: Display employees who are in the HR or Finance department,
--or whose name contains at least 4 vowels.
--Tables Used: Employees, Departments

SELECT * FROM Employees
SELECT* FROM Departments
SELECT name as EmployeeName,DepartmentName FROM Employees E JOIN Departments D ON E.DepartmentID=D.DepartmentID
WHERE DepartmentName='HR' OR DepartmentName='Finance' or name like '____'

 --q19
--Return: EmployeeName, DepartmentName, Salary
--Task: Show employees who are in the Sales or
--Marketing department and have a salary above 60000.
--Tables Used: Employees, Departments

select* from Employees
select* from Departments
select name as employeename, DepartmentName, Salary from Employees e join Departments d on e.DepartmentID=d.DepartmentID
where DepartmentName='sales'or DepartmentName='marketing' or Salary>60000
