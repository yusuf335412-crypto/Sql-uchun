...
--Easy-Level Tasks
--q1
--Using Products table, find the total number of products
--available in each category.
select category,count(ProductName) from Products group by Category

--q2
--Using Products table, get the average price of products 
--in the 'Electronics' category.
select * from Products
select category, avg(price) from Products where category='electronics' group by category

--q3
--Using Customers table, list all customers 
--from cities that start with 'L'.
...
--q7
--Using Products table, label stock as 'Low Stock' 
--if quantity < 30, else 'Sufficient'.
select * from Products
select *,
case 
    when stockquantity<30 then 'low stock' else 'sufficent' end as type
from Products
...
--Using Customers table, find the total number of customers in each country.
 ...
 --q10
 --Using Orders and Invoices tables, list customer IDs who placed 
 --orders in 2023 January to find those who did not have invoices.
 select * from Invoices
 select * from Orders

 select *from Invoices as a right join Orders as b on a.CustomerID=b.CustomerID where YEAR(OrderDate)=2023

 --q11
 --Using Products and Products_Discounted table, Combine all product 
 --names from Products and Products_Discounted including duplicates.
 select * from Products
 select * from Products_Discounted

 select productname from Products union all select productname from Products_Discounted

 select count(*) as totalproducts from ( select productname from Products
 union all select productname from Products_Discounted) as allproducts
 
 --q12
 --Using Products and Products_Discounted table, Combine all product 
 --names from Products and Products_Discounted without duplicates.
...
--q14
--Using Products table, group products based on price:
...
--Using City_Population table, use Pivot to show 
--values of Year column in seperate columns ([2012], [2013]) 
--and copy results to a new Population_Each_Year table.

select * from city_population

select * into Population_Each_Year from city_population 
pivot ( sum (population) for year in ([2012],[2013])
 ...
 select productid, sum(productid) as total from sales group by ProductID

 --q17
 --Using Products table, use wildcard to find products 
 --that contain 'oo' in the name. Return productname.
select * from Products where ProductName like '%oo%'

--q18
--Using City_Population table, use Pivot to show 
--values of City column in seperate columns (Bektemir, Chilonzor, Yakkasaroy) 
--and copy results to a new Population_Each_City table.
select * from city_population

select * from city_population pivot ( sum(population) for district_name 
in ([Bektemir], [Chilonzor], [Yakkasaroy])) as p 

select * into  Population_Each_City from city_population 
pivot ( sum(population) for district_name in ([Bektemir], [Chilonzor], [Yakkasaroy])) as p 


--Hard-Level Tasks
--q19
--Using Invoices table, show top 3 customers 
--with the highest total invoice amount. Return CustomerID and Totalspen

select*from Invoices

select top 3 customerid, sum(totalamount) from Invoices group by customerid order by sum(TotalAmount) desc
...
select* from Population_Each_Year
select * from Population_Each_Year
unpivot(Population for year in ([2012],[2013])) as unp

--q21

--Using Products and Sales tables, list product names 
--and the number of times each has been sold. (Research for Joins)

select * from Products
select * from Sales


select productname, stockquantity from Products p join Sales s on p.ProductID=s.ProductID 
...
select * from Population_Each_City 
unpivot( population for district_name in ([Bektemir],[Chilonzor],[Yakkasaroy])) as unp
